    ...
    */
    ...
    if (select_button_event) {
        ...
        */
        ...
        if (request_state==RS_WAIT_FOR_START) {
            ...
            cur_path_len = 0; // clear previous path length
            update_display_window = 1;
        }
            ...
            if (path_len == -2 || path_len == -1) { // error occurred
                tft.setCursor(44, 76);
                tft.setTextColor(BLACK);
                if (path_len == -2) { // timeout error
                    tft.print("Timeout!");
                }
                else { // other communication error
                    tft.print("Error!");
                    dprintf("Error, starting over.");
                }
                Serial.println("GARBAGE!"); // To reset the server
                delay(1000);
                update_display_window = 1;
            }
            else {
            ...
            if ( path_len > 0 ) {
                update_display_window = 1;
                tft.print("Calculating waypoints");
                tft.print("Path length: "); //let the user know the path length on the tft screen
                tft.print(path_len);
                ...
                dprintf("Max path length: %d", max_path_len);
                dprintf("Path length: %d", path_len);

                // determine if waypoints were received successfully
                int diagnostic = srv_get_waypoints(waypoints, path_len, max_path_len);

                if (diagnostic >= 0){ // no error occurred
                    ...
                    cur_path_len = 0; // make sure that no path will be drawn
                    ...
                    Serial.println("GARBAGE!"); // Send garbage to reset the server
                    delay(1000);
                    update_display_window = 1;
                }
            }
                ...
                cur_path_len = 0; // make sure that no path will be drawn
                // print a no path message to the user
                tft.setCursor(44, 76);
                tft.setTextColor(BLACK);
                tft.print("No path!");
            }
    ...
    if (update_display_window) {
        ...
        if ( cur_path_len > 1 ) {
            ...
            // First use longitude_to_x and latitude_to_y to convert lon and lat values to map coordinates
            // then subtract the screen_map values from the map coordinates to get the actual screen coordinates
            int min = -1;
            int y_upper = 161;
            int x_upper = 129;
            for (int16_t i=0; i < cur_path_len-1; i++) {

                // These statements convert a lat or lon value to y or x and then subtract screen_map to get
                // the screen coordinates. Constrain is used to make sure that the values will never be outside
                // the boundaries of the screen (necessary for long paths).
                uint16_t x1 = constrain(longitude_to_x(current_map_num, waypoints[i].lon)-screen_map_x, min, x_upper);
                uint16_t y1 = constrain(latitude_to_y(current_map_num, waypoints[i].lat)-screen_map_y, min, y_upper);
                uint16_t x2 = constrain(longitude_to_x(current_map_num, waypoints[i+1].lon)-screen_map_x, min, x_upper);
                uint16_t y2 = constrain(latitude_to_y(current_map_num, waypoints[i+1].lat)-screen_map_y, min, y_upper);

                if (((x1 != x2 || x1 != min) && (x1 != x2 || x1 != x_upper)) && // checks if path lines are visible on screen
                ((y1 != y2 || y1 != min) && (y1 != y2 || y1 != y_upper))) {
                    tft.drawLine(x1, y1, x2, y2, RED); // only draw the path segment if it is visible
                }
            }
            dprintf("Free mem:%d", AVAIL_MEM);
        }
    ...
    if (request_state == RS_WAIT_FOR_START) {
        ...
        tft.setCursor(0,0);
        tft.setTextColor(BLACK);
        tft.print("Select a path"); // tell user to select the path
    }
    else {
        status_msg("TO?");
    }
